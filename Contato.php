...
class Contato
{
    ...
    public function save()
    {
        ...
        if ($conexao = Conexao::getInstance()) {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->rowCount()) {
                    $this->dadosError($result);
                    return false;
                }
            }
        }
            // query para atuallizar data
        if (!isset($this->id)) {
            $fundir_colunas = array_merge(['data_criacao' => $data->dataCriacao()], $colunas);
            $query = "INSERT INTO contatos (" .
                implode(', ', array_keys($fundir_colunas)) .
                ") VALUES (" .
                implode(', ', array_values($fundir_colunas)) . ");";
        } else {
            $fundir_colunas = array_merge(['data_atualizacao' => $data->dataAtualizacao()], $colunas);

            foreach ($fundir_colunas as $key => $value) {
                if ($key !== 'id') {
                    $definir[] = "{$key}={$value}";
                }
            }
    ...
    public static function destroy($id)
    {
      ...
      /*
        Para não Duplicar os dados
    */
    public function dadosError(array $colunas)
    {
        $recebColuna = implode(',', $colunas);
        $this->mensagem = "existe um erro de registro igual  {$recebColuna}";
    }

    public function getdadosError()
    {
